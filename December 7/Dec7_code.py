...
# to blast a large enough hole for your submarine to get through. 
...
# This is the new cheapest possible outcome; the old alignment position (2) now
...
with open("Dec7_data.txt") as f:
    data = f.readlines()

for row in data:
    crab_positions = [int(val) for val in row.strip().split(",")]

import numpy as np

max_pos = max(crab_positions)

def task1(max_pos, crab_positions):
    """
    Returns the minimum amount of fuel to align assuming a move costs 1 fuel

    Inputs:
        max_pos (int): Maximum position of a crab in crab positions
        crab_positions (list of ints): Initial crab positions
    
    Returns (float): Amount of fuel burned
    """
    crab_grid = np.zeros((max_pos + 1, max_pos + 1))

    for crab in crab_positions:
        crab_deltas = crab_grid[crab, :]
        for i, _ in enumerate(crab_deltas):
            crab_grid[crab, i] += abs(crab - i)

    col_sums = crab_grid.sum(axis=0)
    
    return min(col_sums)

def task2(max_pos, crab_positions):
    """
    Returns the minimum amount of fuel to align assuming a move costs 1
        additional fuel per step (e.g. 1 to 2 is one fuel but 2 to 3 is two
        fuel)
    ...
    for crab in crab_positions:
        ...
        for i, _ in enumerate(crab_deltas):
            dist = abs(crab - i)
            crab_grid[crab, i] += (dist * (dist + 1)) / 2
            # To sum b/t 2 numbers it is (n * (n + 1)) / 2

    col_sums = crab_grid.sum(axis=0)
    
    return min(col_sums)

print(task1(max_pos, crab_positions))
print(task2(max_pos, crab_positions))