...
data = []
with open("Dec4_data.txt") as f:
    data = f.readlines()

calls_str = data[0]
calls = [int(val) for val in calls_str.strip().split(",")]
del data[0:2]

boards = []
board = []
for i, row in enumerate(data):
    int_row = [int(num) for num in row.strip().split(" ") if num != ""]
    if int_row:
...
def task_1(boards, calls):
    """
    Find the score of the winning board
    
    Inputs:
        boards (numpy array): List of boards
        calls (list): Numbers called
    
    Returns:
        Final score of winning board (sum of uncalled numbers * winning number)
    """
    ones = np.ones(boards.shape)
    for call in calls:
        board,x,y = np.where(boards == call)
        locations = []
        ...
        board_nums = []
        for j in board:
            if 0 in np.sum(ones[j], axis = 0) or 0 in np.sum(ones[j], axis = 1):
                board_nums.append(j)
        if board_nums:
            break
    
    output = []
    for board in board_nums:
        to_sum = boards[board] * ones[board]
        sums = to_sum.sum()
        output.append(sums * call)
    return output

def task_2(boards, calls):
    """
    Find the score of the winning board that wins last
    ...
    Returns:
        Final score of last board to win (sum of uncalled numbers * winning num)
    """
    ones = np.ones(boards.shape)
    boards_copy = np.copy(boards)
    for call in calls:
        board,x,y = np.where(boards_copy == call)
        locations = []
        ...
        for j in board:
            if 0 in np.sum(ones[j], axis = 0) or 0 in np.sum(ones[j], axis = 1):
                board_nums.append(j)
        if board_nums and boards_copy.shape[0] != 1:
            for k, num in enumerate(board_nums):
                boards_copy = np.delete(boards_copy, num - k, 0)
                ones = np.delete(ones, num -k, 0)
        elif boards_copy.shape[0] == 1 and board_nums:
            break

    to_sum = boards_copy * ones
    sums = to_sum.sum()
    return sums * call

print(task_1(boards_np, calls))
print(task_2(boards_np, calls))