...
# 
# The power consumption can then be found by multiplying the gamma rate by the 
# epsilon rate.
...
with open("Dec3_data.txt", newline='') as f:
  reader = csv.reader(f)
  row1 = next(reader)
  for val in row1:
      length = len(val)
...
def task_1(length):
    """
    Returns power consumption rate
    
    Input:
        Length (int): Length of binary number
    
    Returns:
        Power consumption (int): Amount of power consumed
    """
    with open("Dec3_data.txt") as f:
        tracker = [0] * length
        row_count = 1
        for row in f:
            row_count += 1
        ...
        for j in range(length):
            if tracker[j] >= row_count / 2: 
                # No instructions for if equal
                gamma += "1"
                epsilon += "0"
            else:
                gamma += "0"
...
def task_2(length):
    ...
    with open("Dec3_data.txt") as f:
        ...
        return helper_2(oxygen_rate, co2_rate, 1, length)


def helper_2(oxygen_list, co2_list, index, max_index):
    """
    Returns list representing oxygen rating and co2 rating
    
    Input:
        oxygen_list (list): list of potential oxygen ratings
        co2_list (list): list of potential co2 ratings
        index (int): index of number
    
    Returns:
        (tuple of lists): oxygen_list, co2_list 
            of 1 value each representing ratings
    """
    oxy_1 = []
    oxy_0 = []
    co2_0 = []
    ...
    assert index + 1 <= max_index, "did not find a number in time"
    ...
    if len(oxygen_list) == 1 and len(co2_list) == 1:
        ox, co2 = int(oxygen_list[0], 2), int(co2_list[0], 2)
        # print(ox * co2)
        return (ox * co2)
    else:
        helper_2(oxygen_list, co2_list, index + 1, max_index)
