...
# to reach such large numbers - maybe exponentially quickly?
...
with open("Dec6_data.txt") as f:
    data = f.readlines()

for row in data:
    fishies = [int(val) for val in row.strip().split(",")]


def task(fishies, target_day, norm_breed, extra_days):
    """
    Returns number of fish in target days based on 7 day breeding (index 0)
    Note: assumes that first time reproduction occurs in 9 days per problem

    Input:
        fishies (list of int): Fish states
        target_day (int): Stop time
        norm_breed (int): Normal breeding time for fish
        extra_days (int): Extra reproduction time for first reproduction  
    
    Returns (int): Number of fish at end of time
    """
    day = 0
    max_time = norm_breed + extra_days
    fish_dict = {k:0 for k in range(0,max_time + 1)}
    for val in fishies:
        fish_dict[val] += 1
    
    while day < target_day:
        fish_dict2 = {k:0 for k in range(0,max_time + 1)}
        for k, v in fish_dict.items():
            if k == 0:
                fish_dict2[max_time] += v
                fish_dict2[norm_breed] += v
            else:
                fish_dict2[k-1] += v
        fish_dict = dict(fish_dict2)
        day += 1
    return sum(fish_dict.values())

print(task(fishies, 80, 6, 2))
print(task(fishies, 256, 6, 2))