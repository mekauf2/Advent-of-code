...
# Each octopus slowly gains energy over time and
# flashes brightly for a moment when its energy is full.
...
def read_data(path):
    """
    Reads in data for octopus locations

    Input:
        path (filepath): location of data
    
    Returns (np.array): Np.array of octopus locations
    """
    with open(path) as f:
        rows = f.readlines()
    ...
    for row in rows:
        ...
        row = row.strip()
        for char in row:
            rows_int.append(int(char))
        data.append(rows_int)

    data_np = np.array(data)

    return data_np

def task_1(path, steps):
    """
    Given a square set of inputs 0-8, returns total flashes that occur (tile
        reaches > 9). Tile increases by 1 each round and flashes increase by
        additional 1
    ...
    Returns:
        total_flashes (int): Number of flashes that occured
    """
    data = read_data(path)
    total_flashes = 0
    x, y = data.shape
    ...
    for _ in range(steps):
        data[:,:] += 1
        to_flash_loc = np.argwhere(data > 9)
        flashes_to_add, _ = advance_one_step(data, to_flash_loc, max_x, max_y)
        total_flashes += flashes_to_add
        data[data > 9] = 0
    return total_flashes

def add_one_to_neighbors(data, location, max_x, max_y):
    """
    Given a set of octopus status, add 1 to each neighbor from a flash
    
    Input:
        data (np.array): octopuses statuses
        location (tuple): x, y location of flash
        max_x (int): maximum x location
        max_y (int): maximum y location
    
    Returns: None (modifies statuses in place)
    """
    x, y = location
    lhs = max(y-1, 0)
    rhs = min(y+1, max_y) + 1
    ...
    data[bottom:top, lhs:rhs] += 1

    return None

def advance_one_step(data, to_flash_loc, max_x, max_y):
    """
    Advances one step in octopus data
    
    Input:
        data (np.array): Octopuses statuses
        to_flash_loc (np.array): Locations of initial flashes
        max_x (int): maximum x location
        max_y (int): maximum y location
        return_choice (int): Which return value to return (0/1)
    
    Returns: flashes_completed (int), to flash_loc (np.array)
    """
    flashes_completed = 0
    to_reset = np.ones((data.shape))
    while len(to_flash_loc) != flashes_completed:
        for i, j in to_flash_loc:
            if to_reset[i,j] != 0:
                add_one_to_neighbors(data, (i,j), max_x, max_y)
                to_reset[i,j] = 0
                flashes_completed += 1
        to_flash_loc = np.argwhere(data > 9)

    return flashes_completed, to_flash_loc


def task_2(path):
    """
    Given a square set of inputs 0-8, returns the step where all tiles flash (tile > 9). 
    Tile increases by 1 each round and flashes increase by additional 1
    ...
    to_flash_loc = []
    steps = 0
    while len(to_flash_loc) != data.size:
        steps +=1
        data[:,:] += 1
        to_flash_loc = np.argwhere(data > 9)
        _, to_flash_loc = advance_one_step(data, to_flash_loc, max_x, max_y)
        data[data > 9] = 0
    return steps

print(task_1("Dec11_data.txt", 100))
print(task_2("Dec11_data.txt"))
