...
# In the above example, the priority of the item type that
# appears in both compartments of each rucksack is 16 (p), 38 (L), 42 (P), 22 (v), 20 (t), and 19 (s);
# the sum of these is 157.

# Find the item type that appears in both compartments of each rucksack.
# What is the sum of the priorities of those item types?

# Note: ord converts character to number
# ord("a") = 97. To make 1 based, subtract 96
# ord("A") = 65. To make 27 based, subtract 38
...
# item type B somewhere in their rucksack, and at most two of the Elves will be
# carrying any other item type.

# The problem is that someone forgot to put this year's updated authenticity
# sticker on the badges.
# All of the badges need to be pulled out of the rucksacks so the new authenticity
...
with open("input.txt") as f:
    ...
    while True:
        ...
        if not line:
            break
        # Once reach 3rd elf, check prior elves for right item
        if i % 3 == 2:
            for item in line:
                if item in lines[0] and item in lines[1]:
                    break
            if ord(item) > 96:
                total += ord(item) - 96
            else:
                total += ord(item) - 38
        # If have reach a new set of 3 elves, reset list, otherwise append
        if i % 3 == 0:
            lines = [line.strip()]
        else:
