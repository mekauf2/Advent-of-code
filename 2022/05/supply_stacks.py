...
with open("input.txt") as f:
    ...
    while True:
        ...
        if not integer_check:
            ...
            stack = 1
            for char in line:
                # Check if integer, as that tells us we're going to read the move piece
                ...
                if char == " ":
                    ...
                    stacks[stack] = [char] + stacks.get(stack, [])
                    stack += 1
                    spaces = -1
                if spaces == 3:
                    stack += 1
            ...
            move_num = int(line_part[1])
            from_col = int(line_part[3])
            to_col = int(line_part[5].strip())
            for _ in range(move_num):
                box = stacks[from_col].pop()
                stacks[to_col] = stacks[to_col] + [box]
    # print(stacks)
...
with open("input.txt") as f:
    ...
    while True:
        ...
        if not integer_check:
            ...
            from_col = int(line_part[3])
            to_col = int(line_part[5].strip())
            boxes = stacks[from_col][-move_num:]
            stacks[from_col] = stacks[from_col][:-move_num]
            stacks[to_col] = stacks[to_col] + boxes
    # print(stacks)
    for i in range(1, len(stacks)+1):
        print(stacks[i][-1])
