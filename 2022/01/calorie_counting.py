...
def check_elves(top_elves, elf_to_check):
    '''
    '''
    for i, elf in enumerate(top_elves):
        if elf_to_check > elf:
            for j in range(len(top_elves)-1, i, -1):
                top_elves[j] = top_elves[j-1]
            top_elves[i] = elf_to_check
            break
    return top_elves

number_elves = 3 # Note: can do task 1 by setting = 1
best_total = [0] * number_elves
current_elf = 0
with open("input.txt") as file:
    while True:
        line = file.readline()
        if not line:
            best_total = check_elves(best_total, current_elf)
            break
        if line != "\n":
            ...
            best_total = check_elves(best_total, current_elf)
            current_elf = 0
print(sum(best_total), best_total)
