...
# sea floor depth as the sweep looks further and further away from the sub
...
def task_1(vals):
    """
    Counts the number of values > prior value
    Input (list): list of values
    Returns (int): Count of values > prior values 
    """
    prior = float("inf")
    tracker = 0
    for val in vals:
...
print("Task 1 count:", task_1(vals))
...
# The sum of measurements in the second window is larger than the sum of the
...
def task_2(vals):
    """
    Counts the number of times the sum of a window of 3 numbers increases
    Input (list): List of values
    Returns (int): Count of increases
    """
    ...
    for i, val in enumerate(vals):
        if i < 3:
            window.append(val)
            if i == 2:
                prior_window_sum = sum(window)
        else:
            window[i%3] = val
            new_window_sum = sum(window)
            if new_window_sum > prior_window_sum:
                count += 1
            prior_window_sum = new_window_sum
    return count

print("Task 2 count:", task_2(vals))